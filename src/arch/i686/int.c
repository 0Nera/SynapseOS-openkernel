...
extern void isr80(void);    ///< Вектор прерываний для системных вызовов (НЕ РЕКОМЕНДУЕТСЯ ИСПОЛЬЗОВАТЬ ПРЕРЫВАНИЯ КАК СИСТЕМНЫЕ ВЫЗОВЫ)
 ...
 * 
 */
static noreturn void double_fault(registers_t *regs) {
    com1_log("[ERROR]Double fault, int num: %d", regs->int_num);

    com1_log_dump(regs, sizeof(registers_t));
 ...
 * 
 */
static noreturn void general_protection_error(registers_t *regs) {
 ...
 * 
 */
static noreturn void page_fault(registers_t *regs) {
